...
module Jekyll
  ...
  module WhatIfLocalization
    ...
    LANG_END_RE = %r{#{WhatIfLocalization::LANG_EXT_RE}\z}
  ...
  class Page
    ...
    def url(lang = nil)
      url = "#{_localization_original_url}"
      url = url.split("/")
      url[url.length-1]
    end
    ...
    def destination(dest)
      ...
      if ext == '.html' && _localization_original_url !~ /\.html\z/
        path.sub!(WhatIfLocalization::LANG_END_RE, '')
    ...
    def process(name)
      ...
      self.basename = name[0 .. -self.ext.length-1].
        sub(WhatIfLocalization::LANG_END_RE, '')
    end
  ...
  module Filters
    ...
    def other_langs
      WhatIfLocalization::LANGUAGES - [lang]
    end

    def human_lang(lang = lang)
      translate(*WhatIfLocalization::HUMAN_LANGUAGES[lang]) || lang.capitalize
    end

    def native_lang(lang = lang)
      translate_lang(lang, *WhatIfLocalization::HUMAN_LANGUAGES[lang]) || lang.capitalize
    end

    def url_lang(url, lang = lang)
      url = url.sub(WhatIfLocalization::LANG_END_RE, '')
      url << '.' << lang if lang
      url
    end
    ...
    # the current language's position in the WhatIfLocalization::LANGUAGES array.
    ...
    def translate_lang(lang, *translations)
      ...
      if translations.last.is_a?(Hash)
        ...
        if translations.empty?
          ...
          hash[key = WhatIfLocalization::LANGUAGES.first] || hash[key.to_sym]
        else
          raise ArgumentError, 'both hash and array given'
        end
      else
        index = WhatIfLocalization::LANGUAGES.index(lang)
    ...
    def date_to_string(date, lang = lang)
      local_date_string(date, WhatIfLocalization::DATE_FMT_SHORT[lang], lang)
    end
    ...
    def date_to_long_string(date, lang = lang)
      local_date_string(date, WhatIfLocalization::DATE_FMT_LONG[lang], lang)
    end
    ...
    def local_date_string(date, fmt, lang = lang)
      ...
      fmt.scan(/%[EO]?(.)|(.)/) { |a, b|
        res << case a
          when nil then b
          when 'A' then WhatIfLocalization::DAYNAMES[lang][date.wday]
          when 'a' then WhatIfLocalization::ABBR_DAYNAMES[lang][date.wday]
          when 'B' then WhatIfLocalization::MONTHNAMES[lang][date.month]
          when 'b' then WhatIfLocalization::ABBR_MONTHNAMES[lang][date.month]
          when 'c' then local_date_string(date, WhatIfLocalization::TIME_FMT[lang], lang)
          when 'P' then WhatIfLocalization::MERIDIAN[lang][date.send(:hour) < 12 ? 0 : 1].downcase
          when 'p' then WhatIfLocalization::MERIDIAN[lang][date.send(:hour) < 12 ? 0 : 1]
          else '%' << a
        end
      } if fmt
